...
/// Without a lock + isolation, env-based tests can race and become flaky.
...
///
/// We clear both `PREFIX__*` and `PREFIX_*` because some loaders treat `_` as a prefix
/// separator even when `separator("__")` is used for nested keys.
