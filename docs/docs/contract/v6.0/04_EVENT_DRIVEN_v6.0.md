# Contract v6.0 — Event-driven system
...
## 2. Event Envelope v6.0 ⚠️ BREAKING CHANGE
...
EventEnvelopeV5.2:
  ...
  # Residency & Security
  policy:                          # ⚠️ NEW: purpose/consent metadata (Rhelma6)
    purpose: string                # Declared purpose (e.g. chat | moderation | analytics)
    data_tier: enum                # public_commons | consent_based | sensitive_jury_gated | private_no_train
    consent: bool                  # Required true when data_tier=consent_based
    retention_seconds: int?        # Optional TTL for downstream storage
    policy_hash: string?           # Optional policy ruleset pin
...
- `policy` MUST be present and immutable (purpose + data tier + consent + retention)
- If `policy.data_tier=consent_based`, `policy.consent` MUST be true (reject otherwise)
- If `policy.data_tier=private_no_train`, training/distillation consumers MUST reject the event
...
### Purpose Registry (Service-level Standard)

To avoid *purpose-washing* and to make event streams queryable, Rhelma6 defines a canonical **purpose registry**.
Each service MUST publish with its own stable purpose string.

**Built-in purposes (v6):**

- `ai_orch` — AI Orchestrator control-plane events
- `auth` — Auth service events
- `search` — Search service events
- `search_analytics` — Search analytics / telemetry events
- `realtime` — Realtime service events
- `patch_applier` — Patch Applier worker events
- `sandbox_runner` — Sandbox Runner worker events
- `ops_audit` — Audit / compliance events

**Enforcement switches:**

- `RHELMA_POLICY_REQUIRE_PURPOSE=1` → reject envelopes with `policy.purpose=unspecified`
- `RHELMA_POLICY_ENFORCE_PURPOSE_REGISTRY=1` → reject envelopes whose purpose is not registered
- `RHELMA_POLICY_PURPOSE_REGISTRY_EXTRA="foo,bar"` → extend the registry with additional purposes
- `RHELMA_POLICY_PURPOSE_FROM_TOPIC_FALLBACK=1` → if purpose is `unspecified`, derive it from `topic` prefix (safe legacy fallback)
- `RHELMA_POLICY_PURPOSE_TOPIC_MAP_EXTRA="prefix=purpose,..."` → add/override topic→purpose prefix mappings
...
### 4.2 AI Incident Events (NEW in v6.0)
...
### 4.3 AI Command Events (NEW in v6.0)
...
See **06_DISTRIBUTED_TRANSACTIONS_v6.0.md** for complete saga events.
...
See **07_DATA_LAYER_v6.0.md** for complete vector events.
...
A service is **Event-Driven v6.0 Compliant** if:

✅ Uses EventEnvelope v6.0  
...
**End of Event-Driven Architecture v6.0**