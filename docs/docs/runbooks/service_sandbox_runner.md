# Service Runbook: sandbox-runner (Rhelma6)

## What this service does

sandbox-runner is a Rhelma6 component. For intent, configuration, and API/event contracts, see:
...
```bash
kubectl get pods -n rhelma6 -l app=sandbox-runner -o wide
kubectl logs -n rhelma6 -l app=sandbox-runner --tail=200
...
```bash
kubectl rollout restart deploy/sandbox-runner -n rhelma6
kubectl rollout status deploy/sandbox-runner -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/sandbox-runner -n rhelma6
kubectl rollout status deploy/sandbox-runner -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/sandbox-runner -n rhelma6 --replicas=4
  ```
