# Service Runbook: security-governance (Rhelma6)

## What this service does

security-governance is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/security-governance/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=security-governance -o wide
kubectl logs -n rhelma6 -l app=security-governance --tail=200
...
```bash
kubectl rollout restart deploy/security-governance -n rhelma6
kubectl rollout status deploy/security-governance -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/security-governance -n rhelma6
kubectl rollout status deploy/security-governance -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/security-governance -n rhelma6 --replicas=4
  ```
