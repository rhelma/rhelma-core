# Service Runbook: mni-rag (Rhelma6)

## What this service does

mni-rag is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/mni-rag/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=mni-rag -o wide
kubectl logs -n rhelma6 -l app=mni-rag --tail=200
...
```bash
kubectl rollout restart deploy/mni-rag -n rhelma6
kubectl rollout status deploy/mni-rag -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/mni-rag -n rhelma6
kubectl rollout status deploy/mni-rag -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/mni-rag -n rhelma6 --replicas=4
  ```
