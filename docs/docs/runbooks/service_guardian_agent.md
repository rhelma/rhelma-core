# Service Runbook: guardian-agent (Rhelma6)

## What this service does

guardian-agent is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/guardian-agent/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=guardian-agent -o wide
kubectl logs -n rhelma6 -l app=guardian-agent --tail=200
...
```bash
kubectl rollout restart deploy/guardian-agent -n rhelma6
kubectl rollout status deploy/guardian-agent -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/guardian-agent -n rhelma6
kubectl rollout status deploy/guardian-agent -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/guardian-agent -n rhelma6 --replicas=4
  ```
