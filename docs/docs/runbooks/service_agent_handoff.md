# Service Runbook: agent-handoff (Rhelma6)

## What this service does

agent-handoff is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/agent-handoff/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=agent-handoff -o wide
kubectl logs -n rhelma6 -l app=agent-handoff --tail=200
...
```bash
kubectl rollout restart deploy/agent-handoff -n rhelma6
kubectl rollout status deploy/agent-handoff -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/agent-handoff -n rhelma6
kubectl rollout status deploy/agent-handoff -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/agent-handoff -n rhelma6 --replicas=4
  ```
