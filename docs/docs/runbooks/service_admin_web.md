# Service Runbook: admin-web (Rhelma6)

## What this service does

admin-web is a Rhelma6 component. For intent, configuration, and API/event contracts, see:
...
```bash
kubectl get pods -n rhelma6 -l app=admin-web -o wide
kubectl logs -n rhelma6 -l app=admin-web --tail=200
...
```bash
kubectl rollout restart deploy/admin-web -n rhelma6
kubectl rollout status deploy/admin-web -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/admin-web -n rhelma6
kubectl rollout status deploy/admin-web -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/admin-web -n rhelma6 --replicas=4
  ```
