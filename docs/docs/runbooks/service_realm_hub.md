# Service Runbook: realm-hub (Rhelma6)

## What this service does

realm-hub is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/realm-hub/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=realm-hub -o wide
kubectl logs -n rhelma6 -l app=realm-hub --tail=200
...
```bash
kubectl rollout restart deploy/realm-hub -n rhelma6
kubectl rollout status deploy/realm-hub -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/realm-hub -n rhelma6
kubectl rollout status deploy/realm-hub -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/realm-hub -n rhelma6 --replicas=4
  ```
