# Service Runbook: file-storage (Rhelma6)

## What this service does

file-storage is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/file-storage/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=file-storage -o wide
kubectl logs -n rhelma6 -l app=file-storage --tail=200
...
```bash
kubectl rollout restart deploy/file-storage -n rhelma6
kubectl rollout status deploy/file-storage -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/file-storage -n rhelma6
kubectl rollout status deploy/file-storage -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/file-storage -n rhelma6 --replicas=4
  ```
