# Service Runbook: multi-frontend (Rhelma6)

## What this service does

multi-frontend is a Rhelma6 component. For intent, configuration, and API/event contracts, see:
...
```bash
kubectl get pods -n rhelma6 -l app=multi-frontend -o wide
kubectl logs -n rhelma6 -l app=multi-frontend --tail=200
...
```bash
kubectl rollout restart deploy/multi-frontend -n rhelma6
kubectl rollout status deploy/multi-frontend -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/multi-frontend -n rhelma6
kubectl rollout status deploy/multi-frontend -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/multi-frontend -n rhelma6 --replicas=4
  ```
