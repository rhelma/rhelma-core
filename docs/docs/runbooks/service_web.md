# Service Runbook: web (Rhelma6)

## What this service does

web is a Rhelma6 component. For intent, configuration, and API/event contracts, see:
...
```bash
kubectl get pods -n rhelma6 -l app=web -o wide
kubectl logs -n rhelma6 -l app=web --tail=200
...
```bash
kubectl rollout restart deploy/web -n rhelma6
kubectl rollout status deploy/web -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/web -n rhelma6
kubectl rollout status deploy/web -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/web -n rhelma6 --replicas=4
  ```
