# Service Runbook: patch-applier (Rhelma6)

## What this service does

patch-applier is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/patch-applier/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=patch-applier -o wide
kubectl logs -n rhelma6 -l app=patch-applier --tail=200
...
```bash
kubectl rollout restart deploy/patch-applier -n rhelma6
kubectl rollout status deploy/patch-applier -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/patch-applier -n rhelma6
kubectl rollout status deploy/patch-applier -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/patch-applier -n rhelma6 --replicas=4
  ```
