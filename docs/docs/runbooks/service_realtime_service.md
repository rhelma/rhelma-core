# Service Runbook: realtime-service (Rhelma6)

## What this service does

realtime-service is a Rhelma6 component. For intent, configuration, and API/event contracts, see:
...
```bash
kubectl get pods -n rhelma6 -l app=realtime-service -o wide
kubectl logs -n rhelma6 -l app=realtime-service --tail=200
...
```bash
kubectl rollout restart deploy/realtime-service -n rhelma6
kubectl rollout status deploy/realtime-service -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/realtime-service -n rhelma6
kubectl rollout status deploy/realtime-service -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/realtime-service -n rhelma6 --replicas=4
  ```
