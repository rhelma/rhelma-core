# Service Runbook: bridge-adapter (Rhelma6)

## What this service does

bridge-adapter is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/bridge-adapter/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=bridge-adapter -o wide
kubectl logs -n rhelma6 -l app=bridge-adapter --tail=200
...
```bash
kubectl rollout restart deploy/bridge-adapter -n rhelma6
kubectl rollout status deploy/bridge-adapter -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/bridge-adapter -n rhelma6
kubectl rollout status deploy/bridge-adapter -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/bridge-adapter -n rhelma6 --replicas=4
  ```
