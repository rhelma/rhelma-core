# Service Runbook: rhelma-attestation-verifier (Rhelma6)

## What this service does

rhelma-attestation-verifier is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/rhelma-attestation-verifier/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=rhelma-attestation-verifier -o wide
kubectl logs -n rhelma6 -l app=rhelma-attestation-verifier --tail=200
...
```bash
kubectl rollout restart deploy/rhelma-attestation-verifier -n rhelma6
kubectl rollout status deploy/rhelma-attestation-verifier -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/rhelma-attestation-verifier -n rhelma6
kubectl rollout status deploy/rhelma-attestation-verifier -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/rhelma-attestation-verifier -n rhelma6 --replicas=4
  ```
