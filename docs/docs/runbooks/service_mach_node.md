# Service Runbook: rhelma-node (Rhelma6)

## What this service does

rhelma-node is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/rhelma-node/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=rhelma-node -o wide
kubectl logs -n rhelma6 -l app=rhelma-node --tail=200
...
```bash
kubectl rollout restart deploy/rhelma-node -n rhelma6
kubectl rollout status deploy/rhelma-node -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/rhelma-node -n rhelma6
kubectl rollout status deploy/rhelma-node -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/rhelma-node -n rhelma6 --replicas=4
  ```
