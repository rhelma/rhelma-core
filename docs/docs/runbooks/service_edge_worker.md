# Service Runbook: edge-worker (Rhelma6)

## What this service does

edge-worker is a Rhelma6 component. For intent, configuration, and API/event contracts, see:
...
```bash
kubectl get pods -n rhelma6 -l app=edge-worker -o wide
kubectl logs -n rhelma6 -l app=edge-worker --tail=200
...
```bash
kubectl rollout restart deploy/edge-worker -n rhelma6
kubectl rollout status deploy/edge-worker -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/edge-worker -n rhelma6
kubectl rollout status deploy/edge-worker -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/edge-worker -n rhelma6 --replicas=4
  ```
