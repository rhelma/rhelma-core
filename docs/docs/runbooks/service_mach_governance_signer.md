# Service Runbook: rhelma-governance-signer (Rhelma6)

## What this service does

rhelma-governance-signer is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/rhelma-governance-signer/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=rhelma-governance-signer -o wide
kubectl logs -n rhelma6 -l app=rhelma-governance-signer --tail=200
...
```bash
kubectl rollout restart deploy/rhelma-governance-signer -n rhelma6
kubectl rollout status deploy/rhelma-governance-signer -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/rhelma-governance-signer -n rhelma6
kubectl rollout status deploy/rhelma-governance-signer -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/rhelma-governance-signer -n rhelma6 --replicas=4
  ```
