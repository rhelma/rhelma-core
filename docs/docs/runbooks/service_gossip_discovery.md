# Service Runbook: gossip-discovery (Rhelma6)

## What this service does

gossip-discovery is a Rhelma6 component. For intent, configuration, and API/event contracts, see:

- `apps/gossip-discovery/README.md`
...
```bash
kubectl get pods -n rhelma6 -l app=gossip-discovery -o wide
kubectl logs -n rhelma6 -l app=gossip-discovery --tail=200
...
```bash
kubectl rollout restart deploy/gossip-discovery -n rhelma6
kubectl rollout status deploy/gossip-discovery -n rhelma6 --timeout=180s
```

### Roll back a bad release

```bash
kubectl rollout undo deploy/gossip-discovery -n rhelma6
kubectl rollout status deploy/gossip-discovery -n rhelma6 --timeout=180s
```
...
- Scale horizontally:
  ```bash
  kubectl scale deploy/gossip-discovery -n rhelma6 --replicas=4
  ```
